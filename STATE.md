...
Last run: 2026-08-10 — **Karaoke mobile fs2 (fullscreen iOS + espaçamento)** (branch `feat/karaoke-mobile-fs2`, worktree play-music-fs2) — PR #25 draft aguardando aprovação de merge/deploy

## High Priority (loop is acting or waiting on human)

0. **Karaoke mobile: fullscreen iOS confiável + espaçamento inferior — PR #25 (draft) aguardando aprovação** → branch `feat/karaoke-mobile-fs2` (worktree play-music-fs2, commit 3cac955). (1) Bug iOS: `webkitEnterFullscreen()` era chamado antes do `src` existir (open() antes de loadKaraoke) → falha silenciosa, vídeo ficava inline. Fix: `enterIOSFullscreen()` agenda (`iosFsQueued`) e dispara no `loadedmetadata`/`canplay`. (2) Espaçamento: padding inferior dos controles de 10px → `calc(20px + safe-area)` (24px no mobile) — barra não toca mais a borda inferior. Validação Playwright mobile 375x667: fullscreen ativo, padding 24px, gap borda inferior 24px, sem overflow, ⛶ dentro, zero erros de console; go test verde. **PENDENTE**: aprovação para merge + deploy no :4533.
