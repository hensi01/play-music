...
Last run: 2026-08-10 — **PWA install aviso 1x/sessão** (branch `feat/pwa-once-per-session`, worktree play-music-pwa) — PR #24 draft aguardando aprovação de merge/deploy

## High Priority (loop is acting or waiting on human)

0. **Aviso de instalação PWA — uma vez por sessão — PR #24 (draft) aguardando aprovação** → branch `feat/pwa-once-per-session` (worktree play-music-pwa, commit 6575b62). Fecha a dívida B5 (overlay PWA re-exibia a cada load). `pwa.js`: flag `pm_pwa_seen` em sessionStorage gravada no 1º show; `show()` retorna cedo se já visto na sessão (reload/navegação não re-exibem); fechar aba = nova sessão → aviso volta; try/catch no storage (fallback: comportamento antigo). Validação Playwright :4536: 1º load mostra + flag gravado; reload mesma sessão NÃO mostra; sessão limpa mostra de novo; zero erros de console; go test verde; node --check ok. **PENDENTE**: aprovação para merge + deploy no :4533.
