...
1. This creates a namespace called `playmusic`. Adjust this as needed.
...
1. The `PersistentVolumeClaim` sets up a 2Gi volume for Play Music's database. Adjust this as needed.
1. Be sure to change the `image` tag from `ghcr.io/hensi01/play-music:0.49.3` to whatever the newest version is.
...
1. The `Ingress` presents the service at `playmusic.${SECRET_INTERNAL_DOMAIN_NAME}`, which needs to already be setup in DNS.
